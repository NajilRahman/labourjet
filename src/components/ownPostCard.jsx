...
function OwnPostCard({post,userData,render,reRender}) {
  ...
  const handleEditPost = () => {
    ...
    putData('editPost', { _id: post._id, description: editDesc })
      .then(res => {
        toast.success('Post updated successfully');
        setShowEdit(false);
        if (reRender) reRender(res);
        if (render) render(res);
      })
      .catch(err => {
  ...
  return (
    <div >
          ...
          <Card.Body className='border-0 pt-3'>
            ...
            <div className='d-flex justify-content-start align-items-center gap-3 mt-2'>
              ...
              <div className="dropdown ms-auto">
                <button
                  className="btn btn-light d-flex justify-content-center align-items-center py-2"
                  style={{borderRadius: '25px', width: '50px'}}
                  type="button"
                  id="dropdownMenuButton"
                  data-bs-toggle="dropdown"
                  aria-expanded="false"
                >
                  <i className="fa-solid fa-ellipsis-vertical fa-lg text-white"></i>
                </button>
                <ul className="dropdown-menu dropdown-menu-end bg-glass shadow" aria-labelledby="dropdownMenuButton" style={{border: '1px solid rgba(255,255,255,0.1)'}}>
                  <li>
                    <a className="dropdown-item text-white d-flex align-items-center gap-2" href="#" onClick={deletePost}>
                      <i className="fa-solid fa-trash text-danger"></i> Delete Post
                    </a>
                  </li>
                  <li>
                    <a className="dropdown-item text-white d-flex align-items-center gap-2" href="#" onClick={(e) => { e.preventDefault(); setShowEdit(true); }}>
                      <i className="fa-regular fa-pen-to-square text-success"></i> Edit Post
                    </a>
                  </li>
                </ul>
              </div>
        ...
        <Modal show={showEdit} centered onHide={() => setShowEdit(false)} contentClassName="modal-glass border-0 shadow-lg" backdropClassName="glass-backdrop">
          <Modal.Header closeButton closeVariant="white" className="border-0 pt-4 pb-3 px-4">
            <Modal.Title className="fw-bold text-white">Edit Post</Modal.Title>
          </Modal.Header>
          <Modal.Body className="px-4 py-3">
            <Form.Group>
              <Form.Label className="text-white opacity-75 mb-2">Edit your post description</Form.Label>
              <Form.Control
                as="textarea"
                rows={4}
                value={editDesc}
                onChange={(e) => setEditDesc(e.target.value)}
                className="form-control border-0 px-3 py-2 text-white"
                style={{ backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: '12px' }}
              />
            </Form.Group>
          </Modal.Body>
          <Modal.Footer className="border-0 p-3 px-4 gap-2">
            <Button variant="secondary" className="rounded-pill px-4 border-0" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }} onClick={() => setShowEdit(false)}>
              Cancel
            </Button>
            <Button variant="primary" className="rounded-pill px-4 border-0" onClick={handleEditPost}>
              Save Changes
            </Button>
          </Modal.Footer>
        </Modal>
